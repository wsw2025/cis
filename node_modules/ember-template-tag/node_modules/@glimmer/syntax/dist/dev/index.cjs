...
function parseProgramBlockParamsLocs(code, block) {
  const blockRange = [block.loc.getStart().offset, block.loc.getEnd().offset];
...
function parseElementPartLocs(code, element) {
  const elementRange = [element.loc.getStart().offset, element.loc.getEnd().offset];
...
function parseBlockParams(element) {
  ...
  let attrNames = [];
  for (let i = 0; i < l; i++) {
    attrNames.push(util.unwrap(element.attributes[i]).name);
  }
  let asIndex = attrNames.indexOf('as');
  if (asIndex === -1 && attrNames.length > 0 && util.unwrap(attrNames[attrNames.length - 1]).charAt(0) === '|') {
  ...
  if (asIndex !== -1 && l > asIndex && util.unwrap(attrNames[asIndex + 1]).charAt(0) === '|') {
    // Some basic validation, since we're doing the parsing ourselves
    let paramsString = attrNames.slice(asIndex).join(' ');
    if (paramsString.charAt(paramsString.length - 1) !== '|' || util.expect(paramsString.match(/\|/gu), `block params must exist here`).length !== 2) {
      throw generateSyntaxError("Invalid block parameters syntax, '" + paramsString + "'", element.loc);
    }
    let params = [];
    for (let i = asIndex + 1; i < l; i++) {
      let param = util.unwrap(attrNames[i]).replace(/\|/gu, '');
      if (param !== '') {
...
function buildElement(tag, options = {}) {
  ...
  return {
    ...
    attributes: attrs || [],
    blockParams: blockParams || [],
    blockParamNodes: blockParams?.map(x => ({
      type: 'BlockParam',
      value: x
    })) || [],
    modifiers: modifiers || [],
...
function buildPath(path, loc) {
  if (typeof path !== 'string') {
    if ('type' in path) {
      ...
      util.assert(tail.length === 0, `builder.path({ head, tail }) should not be called with a head with dots in it`);
      let {
        original: originalHead
      } = headToString$1(head);
...
function buildBlockItself(body, blockParams, chained = false, loc) {
  return {
    ...
    body: body || [],
    blockParams: blockParams || [],
    blockParamNodes: blockParams?.map(b => ({
      type: 'BlockParam',
      value: b
    })) || [],
    chained,
...
class PathExpressionImplV1 {
  ...
  get head() {
    ...
    if (this.this) {
      ...
      firstPart = `@${util.getFirst(util.asPresentArray(this.parts))}`;
    } else {
      util.assertPresentArray(this.parts);
      firstPart = util.getFirst(this.parts);
    }
...
class HandlebarsNodeVisitors extends Parser {
  ...
  Program(program) {
    ...
    for (i = 0; i < l; i++) {
      this.acceptNode(util.unwrap(program.body[i]));
    }
...
class TokenizerEventHandlers extends HandlebarsNodeVisitors {
  ...
  finishStartTag() {
    ...
    element.nameNode = {
      type: 'ElementNameNode',
      value: name,
      loc: loc.withStart(this.source.offsetFor(loc.startPosition.line, loc.startPosition.column + 1)).withEnd(this.source.offsetFor(loc.startPosition.line, loc.startPosition.column + 1 + name.length))
    };
